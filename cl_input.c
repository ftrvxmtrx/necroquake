...
KeyDown(kbutton_t *b)
{
	...
	c = Cmd_Argv(1);
	k = c[0] ? strtol(c, NULL, 0) : -1; // -1 == typed manually at the console for continuous down
...
static void IN_Impulse (void) { in_impulse = strtol(Cmd_Argv(1), NULL, 0); }
...
CL_KeyState(kbutton_t *key)
{
	...
	if (impulsedown && !impulseup && down) // pressed and held this frame
		val = 0.5;
	if (!impulsedown && !impulseup && down) // held the entire frame OR up the entire frame
		val = 1.0;
	if (impulsedown && impulseup) // released and re-pressed this frame OR pressed and released this frame
		val = down ? 0.75 : 0.25;
